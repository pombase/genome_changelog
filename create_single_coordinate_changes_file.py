...
data = pandas.DataFrame(all_lines,columns=['revision', 'user', 'date', 'chromosome', 'systematic_id', 'primary_name', 'feature_type', 'added_or_removed', 'value'])

if not ('svn_2' in set(data['revision'])):
    data.revision = data.revision.astype(int)

data.sort_values(['revision','chromosome','systematic_id','feature_type','added_or_removed'],inplace=True, ascending=[False,True, True, True, True])
