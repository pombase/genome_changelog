...
added_rows = data.loc[data.added_or_removed == 'added', ['systematic_id', 'value', 'db_xref']].copy()
# Aggregate the db_xref col
added_rows = added_rows.groupby(['systematic_id', 'value'], as_index=False).agg({'db_xref': agg_function})
...
data.rename(columns={'db_xref': 'db_xref_removed'}, inplace=True)
added_rows.rename(columns={'db_xref': 'db_xref_added'}, inplace=True)
data = data.merge(added_rows, on=['systematic_id', 'value'])

# Re-sort columns and save
data = data[['chromosome','systematic_id','primary_name','feature_type','value','date','db_xref_removed','db_xref_added','pombase_reference','pombase_comments']]
